...
function studentsName1(){
    return students.filter(function(x){  //filter korbe jader gpa 3 er upor e and x er modde reke di be
        return x.gpa > 3 
    }).map(function(y){  //then name onosare mapping ko re y er modde reke dibos
        return y.name;
    })
}
